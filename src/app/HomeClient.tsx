...
export default function HomeClient({
  ...
  const [allProducts, setAllProducts] = useState<any[]>(() => newestProducts(initialProducts))
  const [articles, setArticles] = useState<any[]>(() => initialArticles.slice(0, 3))
  ...
  // Only run client-side fetch if server-side fetch returned empty lists (fallback/development)
  useEffect(() => {
    if (initialProducts.length === 0 && !initialHero) {
      queueMicrotask(() => loadData())
    }
  }, [initialProducts.length, initialHero])
