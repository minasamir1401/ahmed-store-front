                      ...
                      <div className="lg:col-span-1 space-y-6">
                        <div className="space-y-2">
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b pb-2 border-slate-200/50">
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mr-1">عنوان أو موضوع النصيحة</label>
                            <button type="button" onClick={handleTipAI} disabled={isAILoading || !formData.title} className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-xl font-black text-[10px] flex items-center justify-center gap-1.5 transition-all shadow-md shadow-blue-200 disabled:opacity-50 cursor-pointer w-full sm:w-auto">
                                {isAILoading ? <Loader2 className="animate-spin" size={14} /> : <Sparkles size={14} />}
                                توليد بالذكاء الاصطناعي
                            </button>
                          </div>
                          <input type="text" value={formData.title || ''} onChange={e => setFormData({...formData, title: e.target.value})} className="w-full bg-slate-50 focus:bg-white border border-transparent focus:border-emerald-500/20 rounded-2xl py-4 px-6 font-black text-sm outline-none transition-all text-slate-700" placeholder="اكتب موضوع المقالة أو العنوان هنا..." required />
                        </div>
                        <div className="space-y-2">
                          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mr-1">المحتوى</label>
                          <textarea value={formData.content || ''} onChange={e => setFormData({...formData, content: e.target.value})} className="w-full h-48 bg-slate-50 focus:bg-white border border-transparent focus:border-emerald-500/20 rounded-3xl py-4 px-6 text-sm font-bold outline-none transition-all resize-none text-slate-700" placeholder="اكتب النصيحة الطبية هنا..." required />
                        </div>
                      </div>
                    </div>
    </>
