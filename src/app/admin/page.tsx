    ...
    <div className="min-h-screen bg-gray-50 flex flex-col" dir="rtl">
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <aside className={`
          ...
          ${isModalOpen ? 'md:hidden' : 'md:translate-x-0'}
        `}>
        ...
        <main className={`flex-1 overflow-y-auto transition-all duration-300 ${isSidebarOpen ? 'md:mr-64' : 'md:mr-0'} ${isModalOpen ? 'p-0 bg-white' : 'p-4 md:p-8'}`}>
          {!isModalOpen ? (
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                        <div className="lg:col-span-1 space-y-4">
                          ...
                          <div className="aspect-square bg-gray-50 rounded-[3rem] border-2 border-dashed border-gray-200 flex flex-col items-center justify-center relative overflow-hidden group shadow-inner">
                            {formData.image ? <img src={formData.image} className="w-full h-full object-contain p-4" alt="preview" /> : <Upload size={48} className="text-gray-300" />}
                            <input type="file" onChange={(e) => e.target.files && handleFileUpload(e.target.files[0], 'main')} className="absolute inset-0 opacity-0 cursor-pointer" accept="image/*" />
                          </div>
                            ...
                            <div className="grid grid-cols-4 gap-3">
                              {(formData.images ? formData.images.split(',').filter(Boolean) : []).map((img: string, idx: number) => (
                                <div key={idx} className="relative aspect-square rounded-2xl overflow-hidden border border-gray-200 bg-gray-50 group">
                                  ...
                                  }} className="absolute inset-0 bg-red-500/80 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><X size={16} /></button>
                                </div>
                              ))}
