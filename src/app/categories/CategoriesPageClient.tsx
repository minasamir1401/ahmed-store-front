              ...
              {categories.map((cat) => (
                ...
                >
                  ...
                  <div className="relative z-10 flex flex-col items-center gap-6 p-4 sm:p-8 w-full h-full">
                    {cat.image ? (
                      <div className="w-24 h-24 sm:w-36 sm:h-36 rounded-[2rem] overflow-hidden shadow-sm transition-transform duration-500 group-hover:scale-110 bg-transparent flex items-center justify-center relative">
                        <Image src={cat.image} fill className="object-contain" sizes="(max-width: 640px) 96px, 144px" alt={language === 'en' ? (cat.nameEn || translate(cat.name)) : cat.name} />
                      </div>
                    ) : (
