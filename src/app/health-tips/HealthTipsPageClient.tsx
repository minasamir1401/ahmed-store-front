...
interface HealthTipsPageClientProps {
  initialTips?: any[]
}

export default function HealthTipsPageClient({ initialTips = [] }: HealthTipsPageClientProps) {
  const { t, dir, language, translate } = useLanguage()
  const [posts, setPosts] = React.useState<any[]>(() => {
    return initialTips.map((tip: any) => ({
      id: tip.id,
  ...
  React.useEffect(() => {
    if (initialTips.length === 0) {
      queueMicrotask(() => fetchTips())
    } else {
      // Re-format according to active language state change
      const formatted = initialTips.map((tip: any) => ({
        id: tip.id,
        title: language === 'en' ? (tip.titleEn || tip.title) : tip.title,
        image: tip.image || 'https://placehold.co/800x400/e8f0ed/2e7d5e?text=Health+Tip',
        category: language === 'ar' ? 'نصيحة طبية' : 'Health Tip',
      ...
      queueMicrotask(() => setPosts(formatted))
    }
  }, [language, initialTips])
                    ...
                    {filteredPosts.length === 0 ? (
                      ...
                      filteredPosts.map((post) => (
                        ...
                        <div className="relative aspect-video overflow-hidden">
                          <Image src={post.image} fill className="object-cover group-hover:scale-110 transition-transform duration-700" sizes="(max-width: 768px) 100vw, 33vw" alt={translate(post.title)} />
                    ...
                    {posts.slice(0, 3).map((post, i) => (
                      <div key={i} className={`flex gap-4 group cursor-pointer ${isRtl ? 'flex-row' : 'flex-row-reverse'}`}>
                        <div className="w-20 h-20 rounded-2xl overflow-hidden flex-shrink-0 relative">
                          <Image src={post.image} fill className="object-cover group-hover:scale-110 transition-transform" sizes="80px" alt={translate(post.title)} />
                        </div>
