...
export default function ProductPageClient({ params, initialProduct }: { params: { id: string }, initialProduct: any }) {
  ...
  // Parse dosageCalculator dynamically from product
  ...
  // Determine product type for the smart advisor
  ...
  if (isMagnesium) {
    ...
    healthTip = language === 'ar'
      ? `حالتك النشطة (${generalActivity === 'high' ? 'رياضي' : generalActivity === 'moderate' ? 'نشاط متوسط' : 'نشاط قليل'}) تتطلب ترطيباً مستمراً. تناول مكملاتك الغذائية دائماً بجدول منتظم ويفضل تثبيت المواعيد لزيادة الفعالية.`
      : `Your active state (&{generalActivity === 'high' ? 'athletic' : generalActivity === 'moderate' ? 'moderate activity' : 'low activity'}) requires constant hydration. Take your supplements regularly.`;
  }
        ...
        <div className="max-w-screen-xl mx-auto px-1.5 max-[340px]:px-1 xs:px-4 py-4 sm:py-8 relative">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-16 items-start">
            ...
            <div className="space-y-6 lg:col-start-1 lg:row-start-2 order-3 lg:order-none">
              ...
              {/* ── Smart Health Advisor Sidebar Widget ── */}
              {dbCalculator && dbCalculator.enabled ? (
                <motion.div
